...
function _applyEvt(st,evt){
    ...
    switch(evt.type){
        ...
        case 'XFER':{
            ...
            /* أجرة التحويل النوعي على نفس الزبون — الاتجاه يحدّد لمن تُحسب:
               • 730 → 24 : الأجرة عليه (يدين لنا)  →  +
               • 24 → 730 : الأجرة له (تُضاف لرصيده) →  −
               نقرأ الاتجاه من srcType الموجود في كل حدث (قديم وجديد). */
            if(d.convFee>0){
                const _toEq730=(d.srcType==='ذهب 24');   /* من 24 نحو 730 */
                stUpdDebt(d.from,'دينار', _toEq730 ? -d.convFee : d.convFee);
            }
...
function _fbInitialLoadCore(){
    ...
    /* الزبون: التحميل الكامل يجلب كل الأحداث (3006+) رغم أنه يحتاج ~4 فقط —
       ...
       محلية) وضمان دوري نادر. أول دخول يحتاج نت، وبعدها الأحدث فقط.
       ملاحظة: الزبون قارئ خالص، فلا خطر رفع/حذف — التزايدي يلتقط ما يخصّه. */
    /* (لم يعد يُجبَر على التحميل الكامل — يتبع نفس منطق _incremental أعلاه) */
    /* هامش أمان صغير يلتقط أحداثاً بـts أقدم قليلاً من أجهزة أخرى. كان 7 أيام
       للزبون فيجلب ~1MB كل فتح عند أي تغيّر في المؤشّر العام (أي حركة لأي مستخدم)
       — تسرّب حصة ضخم. الزبون قارئ خالص يحتاج الأحدث فقط، فهامش ساعة يكفي. */
    ...
    if(_incremental){
        _baseRef.child('_meta/lastTs').once('value', function(ms){
            ...
            /* نتخطّى الجلب إن لا جديد. الشرط: إمّا لدينا أحداث كافية (>=5)،
               أو أن المستخدم أُرشف من قبل (جلب كامل ناجح) فأحداثه القليلة طبيعية
               لا ناقصة — حاسم للزبون (قارئ خالص بـ~4 أحداث): بدونه يجلب النطاق
               كاملاً (7 أيام ~1MB) كل فتح = تسرّب حصة ضخم. */
            const _localOk = (_localCount>=5) || _archiveEver;
            if(_localOk && _remoteTs>0 && _remoteTs<=_lastLocalTs){
            ...
            /* يوجد جديد (أو لا مؤشّر بعد أو محلي ناقص غير مؤرشف) — تابع الاستعلام */
            _runEventQuery();
