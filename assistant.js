...
const VA = window.VA = (() => {
    ...
    function init() {
        ...
        /* تنبيه الديون المنسية أُلغي بطلب المستخدم (v112) */
    }
    ...
    function _extractIntent(t) {
        ...
        const currency = /دينار|دج\b/.test(t) ? 'دينار'
                       : /دولار|\$/.test(t)   ? 'دولار'
                       /* لانقو (بكل رسومها) = ذهب 24 · طرونط = ذهب 730 */
                       : /لان[قڨغجگك]و|لينقو/.test(t) ? 'ذهب 24'
                       : /طرونط|ترونط|طرنط|ترنط/.test(t) ? 'ذهب 730'
                       : /ذهب 730|730/.test(t)? 'ذهب 730'
                       : /ذهب 24|24 قيراط|\b24\b/.test(t) ? 'ذهب 24'
                       : null;
    ...
    function _allNames() {
        /* كل مصادر الأسماء: الديون + فواتير الذهب + الرافيناج + الدولار + دبي + السجل + السلف */
        const s = new Set();
        _gl('debts')       .forEach(d => d.c && s.add(d.c));
        _gl('invoices')    .forEach(i => i.c && s.add(i.c));
        _gl('rafInvoices') .forEach(r => r.c && s.add(r.c));
        _gl('dollInvoices').forEach(d => d.c && s.add(d.c));
        _gl('dubaiInvoices').forEach(d => d.c && s.add(d.c));
    ...
    function _normName(s){
        return (s||'').trim()
            ...
            .replace(/[\u064B-\u0652\u0670]/g,'')
            /* ال التعريف ولواحق اللهجة في أول الكلمة: العلمه/لعلمه/الالعلمه → علمه */
            .split(/\s+/).map(w=>w.replace(/^(ال|لل|ل|ا)(?=.{3,})/,'')).join(' ')
            .replace(/ا/g,'')          /* الألف تسقط كلياً: عايب≡عيب، طاهر≡طهر */
            .replace(/\s+/g,' ');
    }
    function _matchName(spoken) {
        ...
        /* مطابقة حرفية فقط (بعد التطبيع: ال/ة/الهمزات) — لا تخمين ضبابي */
    ...
    function parse(raw, _silent) {
        ...
        /* الفائدة الشهرية — أولوية قصوى قبل أي التقاط أسماء */
        if(/فا[يئ]د[ةه]|مد ?لي الفا/.test(t)){
            if(typeof openProfitModal==='function'){openProfitModal();respond('هذه فائدة الشهر');}
            else respond('حدّث التطبيق');
            return true;
        }
    ...
    function checkProactiveAlerts() {
        try {
            /* للمسؤول فقط — لا يظهر أبداً للزبون أو عامل الورشة */
            ...
            /* آخر حركة لكل زبون (من السجل) — لتمييز الديون المنسية */
            const lastTs = {};
            (_gl('ops')||[]).forEach(o => {
                if (o.c && o._ts && (!lastTs[o.c] || o._ts > lastTs[o.c])) lastTs[o.c] = o._ts;
            });
            const MAX = 100000000;                /* السقف: 100,000,000 دج — يظهر ما دونه فقط */
            const STALE_DAYS = 30;                /* بلا حركة منذ 30 يوماً = منسي */
            const staleBefore = Date.now() - STALE_DAYS*24*60*60*1000;

            /* ديون منسية: دين أقل من السقف (الكبيرة تتذكرها بنفسك) ولا حركة حديثة للزبون */
            const high = Object.entries(totals)
                .filter(([n, v]) => v > 0 && v < MAX && (!lastTs[n] || lastTs[n] < staleBefore))
                .sort((a, b) => b[1] - a[1]);
            ...
            const rows = high.map(([n, v]) =>
                ...
                `<span style="color:#ef4444;font-weight:800">${fmt(Math.round(v),0)} دج<br><small style="font-weight:600;color:var(--t2)">آخر حركة: ${lastTs[n]?new Date(lastTs[n]).toLocaleDateString('fr-FR'):'لا حركة مسجّلة'}</small></span></div>`
            ).join('');
            showCard(`
            <div class="va-card-title" style="color:#ef4444">📌 ديون منسية</div>
            <div class="va-card-sub">ديون أقل من 100 000 000 دج بلا حركة منذ ${STALE_DAYS} يوماً</div>
            ...
            const msg = `صاحبي — عند ${topName} دين منسي: ${Math.round(topVal / 10000)} مليون`;
            respond(msg, true);
        } catch(e) {}
    }
    ...
    function _periodData(from){
        ...
        const agg={buy:{w:0,d:0,n:0}, sell:{w:0,d:0,n:0}};
        evs.forEach(e=>{
            ...
            /* البيع/الشراء الحر وتصفية البيع/الشراء بسعر (SETTLE_GSM) — كانت لا تُحتسب */
            else if(e.type==='SETTLE_GSM'){
                /* isBuy مخزّن في data؛ يشمل الحر (freeBuy/freeSell) والتصفية بسعر */
                const _isBuy=d.freeBuy?true:(d.freeSell?false:!!d.isBuy);
            ...
            let w=items.reduce((s,it)=>s+(+it.eq730||+it.w||0),0);
            if(!w) w=+op.eq730||+op.eqW||0;
            /* SETTLE_GSM: الوزن من data.w (مكافئ 730)، المبلغ من cashVal/cashTotal */
            if(e.type==='SETTLE_GSM'&&!w){ const k=+d.k||730; w=(+d.w||0)*(k/730); }
            let dn=+inv.tp||0;
            if(!dn) dn=(+op.total||+op.cashVal||Math.abs(+op.a||0))||items.reduce((s,it)=>s+(+it.total||0),0);
            if(e.type==='SETTLE_GSM'&&!dn) dn=Math.abs(+d.cashTotal||0);
    ...
    function _activityReport(t){
        ...
        /* ═══ حساب البيع شاملاً دبي + دبي المشحون غير المبيوع ═══ */
        const invoices=_gl('invoices')||[], dubaiInvoices=_gl('dubaiInvoices')||[], ops=_gl('ops')||[];
        const _num=n=>{try{const v=(new Function('return typeof '+n+'!=="undefined"?'+n+':0'))();return +v||0;}catch(e){return 0;}};
        const dollarRate=_num('dollarRate'), goldPrice=_num('goldPrice');
        const _shpP=(()=>{ const _o=_gl('ops')||[]; const s=(_o.find(x=>x&&x.t==='شحن'&&(x.su||0)>0)||{}).su; return s||parseFloat(String((window.localStorage&&localStorage.getItem('gp12_shiprate'))||'').replace(',','.'))||0; })();
        const _dedNow=goldPrice*0.001/0.730;
        const _eqW=inv=>((inv.items||[]).reduce((s,it)=>s+((+it.w||0)*(((+it.k||730))/730)),0));
        const _tsOf=inv=>{ const m=String(inv.dt||'').match(/(\d{2})\/(\d{2})\/(\d{4})/); return m?new Date(+m[3],+m[2]-1,+m[1]).getTime():0; };
        /* بيع دبي هذه الفترة — نفس صيغة الأرشيف (تقريب 1000 + خصم التكرير) */
        let dubSellW=0,dubSellDz=0;
        const _lastSell=(ts)=>{ try{ const sells=(_gl('dollInvoices')||[]).filter(d=>d&&d.isBuy===false&&(+d.r>0)); if(!sells.length)return 0; const _t=d=>{const m=String(d.dt||'').match(/(\d{2})\/(\d{2})\/(\d{4})/);return m?new Date(+m[3],+m[2]-1,+m[1]).getTime():0;}; let c=sells.filter(d=>_t(d)<=ts); if(!c.length)c=sells; c.sort((a,b)=>_t(b)-_t(a)); return +c[0].r||0; }catch(e){return 0;} };
        dubaiInvoices.forEach(x=>{ if(_tsOf(x)<from)return; if(!(x.usd>0&&x.w>0))return; const rt=_lastSell(_tsOf(x))||(x.rate||0)||dollarRate; if(!(rt>0))return; const eq730=Math.round((x.w/0.730)*10)/10; const gpr=(x.usd-x.w*_shpP)*rt/eq730/100; const gprR=Math.round((gpr-_dedNow)/1000)*1000; dubSellDz+=gprR*eq730; dubSellW+=eq730; });
        /* البيع الكلي = المحلي (من D) + دبي */
        const totSellW=D.sell.w+dubSellW, totSellDz=D.sell.d+dubSellDz;
        const aS=totSellW>0?totSellDz/totSellW:0;   /* متوسط البيع شاملاً دبي */
        const aB=D.buy.w>0?D.buy.d/D.buy.w:0;
        /* دبي: مشحون غير مبيوع */
        let shippedW=0,soldW=0;
        ops.forEach(o=>{ if(o&&o.t==='شحن'){ const w=+o.w||+o.a||0; if(w>0)shippedW+=w; } });
        dubaiInvoices.forEach(x=>{ soldW+=(+x.w||0); });
        const unsoldW=Math.max(0,shippedW-soldW);
        showCard(`
        ...
        <div class="va-bal-grid">
            ...
            <div class="va-bal-cell"><span class="va-bal-lbl">💵 بيع</span>
                <span style="font-weight:800">${_f(totSellW,2)} غ</span></div>
        ...
        respond(`${label}: شريت ${_f(D.buy.w,1)}غ بمتوسط ${_f(aB,0)}، وبعت ${_f(totSellW,1)}غ (مع دبي) بمتوسط ${_f(aS,0)}، وبقي ${_f(r.w,1)}غ${unsoldW>0?`، ومشحون غير مبيوع ${_f(unsoldW,1)}غ`:''}`);
    }
    ...
    function _rafAllAndSave() {
        ...
        window._doRafSave = async () => {
            let cust = (document.getElementById('_rsVoiceCust')?.value||'').trim();
            /* طابق الاسم المنطوق مع أسماء دفاترك — يكتب بصيغتك المسجلة لا بصيغة المُملي */
            let _matched=false;
            try{const _mr=_matchName(cust); if(_mr&&_mr.ok){cust=_mr.name;_matched=true;}}catch(e){}
            /* اسم لا يطابق قائمتك: استئذان صريح قبل فتح حساب جديد */
            if(cust&&!_matched){
                const ok=await (typeof appConfirm==='function'
                    ?appConfirm(`«${cust}» غير موجود في أسمائك.\n\nأتسجّل الفاتورة باسم جديد؟`,'✔️ اسم جديد')
                    :Promise.resolve(confirm('اسم جديد: '+cust+'؟')));
                if(!ok){respond('صحّح الاسم ثم احفظ');const e=document.getElementById('_rsVoiceCust');if(e){e.focus();e.select&&e.select();}return;}
            }
