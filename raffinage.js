...
let _rafMode='refiner';   /* 'refiner' = إرسال لمصفّي (السلوك الأصلي) | 'customer' = تصفية زبون */
...
function applyRafModeUI(){
    ...
    /* صفوف خاصة بفرع المصفّي (الرصيد السابق + لانقو/صوارد) تُخفى في فرع الزبون */
    ...
    const dsp=_rafMode==='refiner'?'':'none';
    if(rowPrev)rowPrev.style.display=dsp;
    if(rowLbls)rowLbls.style.display=dsp;
...
window.calcRaf=()=>{
    ...
    if(_rafMode==='customer'){
        ...
        if(_rafSettled){
            const short=totalPure>avail24+0.001;
            gEl.textContent=(totalPure>0?'−':'')+fmt(totalPure,2)+' غ '+(short?('⚠️ الكوفر غير كافٍ ('+fmt(avail24,2)+'غ)'):'(من الكوفر)');
            gEl.style.color='var(--rd)';
            dEl.textContent='+'+fmt(totalFee,0)+' دج → السيولة';
            dEl.style.color='var(--g400)';
        }else{
            gEl.textContent=(totalPure>0?'−':'')+fmt(totalPure,2)+' غ (دين علينا للزبون)';
            gEl.style.color='var(--rd)';
            dEl.textContent=totalFee>0?('+'+fmt(totalFee,0)+' دج (أجرة له علينا→لنا)'):'0';
            dEl.style.color='var(--g400)';
        }
    }else{
...
window.saveSimpleRaf=()=>{
    ...
    const barsRemove730=[], barUpdates730=[];
    {
        ...
        const _consume=list=>{
            for(let i=0;i<list.length && rem>0.001;i++){
                ...
                else { barUpdates730.push({id:bar.id,pool:'730',newW:parseFloat((bar.w-rem).toFixed(4))}); rem=0; }
            }
        };
    ...
    const _raf={id:rid,c,rows,sentW:totalSentW,eq24:totalSentEq24,fee:totalDinar,feeRate,sawared,lanqo,prevD,prevG,finalDinar,finalGold,dt};
    emitEvent('RAF',
...
window.saveCustomerRaf=function(){
    const c=document.getElementById('rafCustomer').value.trim();
    ...
    if(settled){
        const avail24=g24.reduce((s,b)=>s+(b.w||0),0);
        if(eq24>avail24+0.001)return toast(`⚠️ مخزون 24 غير كافٍ (متاح: ${fmt(avail24,2)} غ)`,'error');
        /* استهلاك مكافئ 24 من الكوفر — من الأحدث للأقدم */
        let rem=eq24;
        ...
        for(let i=0;i<src.length&&rem>0.001;i++){
            ...
            else{barUpdates24.push({id:bar.id,pool:'24',newW:parseFloat((bar.w-rem).toFixed(4))});rem=0;}
        }
    }
    const rid='RAF-'+uid();
    const nowStr=new Date().toLocaleDateString('fr-FR',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'});
    const _raf={id:rid,c,mode:'customer',settled,rows,sentW:totalW,eq24,fee:totalFee,feeRate,
                sawared:0,lanqo:0,prevD:0,prevG:0,
                finalDinar:totalFee,finalGold:-eq24,dt};
    emitEvent('RAF',
        {mode:'customer',settled,c,rid,rows,totalSentW:totalW,eq24,feeRate,fee:totalFee,
         barsAdd730,barsRemove24,barUpdates24},
        {
            rafInvoice:_raf,
            bars:Object.keys(dispBars).length?dispBars:undefined,
...
function buildRafHtml(r){
    ...
    return`<div style="position:relative;overflow:hidden;padding:14px 18px;font-family:Tajawal,sans-serif;direction:rtl;width:190mm;box-sizing:border-box">
        ...
        <div style="text-align:center;border-bottom:2px solid #c2410c;padding-bottom:8px;margin-bottom:10px">
            <div style="font-size:${hdr+4}px;font-weight:900;color:#c2410c">🔥 ${r.mode==='customer'?'فاتورة تصفية زبون':'فاتورة رافيناج'}${r.mode==='customer'?(r.settled?' — خالصة':' — على الحساب'):''}</div>
