...
function restoreDraft(){
    try{
        ...
        if(d.rows){while(d.rows.length>invRows)addInvRow(true);}
        if(d.rows)d.rows.forEach((r,idx)=>{
            const i=idx+1;
...
/* زر التعميم الدائم: يأخذ أول سعر مُدخل ويعمّمه */
window.invBroadcastPrice=function(){
    let p=0;
    for(let r=1;r<=invRows;r++){
        const v=parseInvNum(document.getElementById(`inv_p_${r}`)?.value);
        if(v>0){p=v;break;}
    }
    if(!(p>0))return toast('أدخل سعراً في أي سطر أولاً','error');
    applyPriceToAll(p);
};
...
window.onInvTypeChange=function(){
    ...
    /* أُلغي عرض اقتراحات سبائك الكوفر فوق الفاتورة — البيع يتم من نافذة المخزون (تحديد ← 🛒 بيع المحدد) */
...
window.sellBarsToInvoice=function(ids){
    ...
    if(sel&&sel.value!=='sell'){sel.value='sell';if(typeof onInvTypeChange==='function')onInvTypeChange();}
    ids.forEach(id=>fillInvRowFromBar('730',id));
    toast(`🛒 أُدرجت ${ids.length>1?ids.length+' سبائك':'السبيكة'} في فاتورة بيع — أدخل الأسعار`,'success');
};
...
function resetInvForm(){
    try{ if(typeof _invPhotos!=='undefined'){_invPhotos=[];_renderInvPhotoStrip&&_renderInvPhotoStrip();} }catch(e){}
...
window.saveInvoice=()=>{
    ...
    resetInvForm();
    if(!_isEdit&&typeof _sendCustomerPush==='function')_sendCustomerPush(c,'فاتورة جديدة','سُجّلت لك فاتورة '+(t==='بيع'?'بيع':'شراء')+' — افتح حسابك للاطلاع');
    toast(_isEdit?'✅ تم تعديل الفاتورة وتحديث الحسابات':'✅ تم حفظ الفاتورة بنجاح');
    /* إشعار للزبون المربوط بحساب (والتطبيق مغلق أيضاً) */
    if(typeof _sendCustomerPush==='function' && !_isEdit)
        _sendCustomerPush(c,'فاتورة '+(t==='buy'?'شراء':'بيع')+' جديدة',
            'سُجّلت لك فاتورة بقيمة '+fmt(tp,0)+' دج — افتح حسابك للتفاصيل');
    setTimeout(()=>printInv(iid),300);
};
window.delInv=async (id)=>{
    if(!(await appConfirm('حذف هذه الفاتورة وعكس أثرها على الحسابات؟','🗑️ حذف')))return;
    if(!_voidByInvId('invoice',id)){
        appAlert('⚠️ هذه الفاتورة قديمة/مرحّلة بلا حدث مؤتمت — حذفها لن يعكس أثرها.\nصحّح آثارها يدوياً إن لزم.');
        return;
    }
    setTimeout(()=>{try{renderArchive();}catch(e){} try{if(typeof updAll==='function')updAll();}catch(e){} try{if(typeof _updBuyAvg==='function')_updBuyAvg();}catch(e){}},300);
...
window.editInv=(id)=>{
    ...
    /* وصل قبض 730: يُعدَّل من نافذة «قبضت» لا من محرّر الفواتير (تختلف دلالته المحاسبية) */
    if(inv.recv){
        if(typeof editRecvInv==='function')return editRecvInv(id);
        toast('🚫 وصل قبض 730 يُعدَّل من نافذة «قبضت»','error');
        return;
    }
    /* فواتير ناتجة عن تصفية/شراء أو بيع حر (لها ملاحظة) — منطقها في دفتر الديون لا هنا */
    if(inv.note){
        toast('🚫 هذه فاتورة ناتجة عن تصفية أو شراء/بيع حر — لا تُعدَّل من هنا. احذفها وأعد تسجيلها من دفتر الديون','error');
        return;
    }
    ...
    invRows=Math.max(10,items.length+1);
    initInvTable();
...
/* ═══════════ صور سبائك فاتورة الشراء (ضغط أقصى) ═══════════ */
let _invPhotos=[];
const INV_MAX_PHOTOS=6;
window.invAddPhotos=function(files){
    [...files].slice(0,INV_MAX_PHOTOS-_invPhotos.length).forEach(f=>{
        const img=new Image();
        img.onload=()=>{
            /* حجم وجودة أعلى — الصورة كانت 500px/0.42 فتظهر باهتة عند المسؤول والزبون */
            /* وضوح أعلى (كان 500px/0.42 باهتاً). الصور في عقدة _photos المنفصلة
               خارج مجرى الأحداث، فلا تُثقل مزامنة الأحداث ولا تملأ تخزينها. */
            const MAX=1000;
            ...
            _invPhotos.push(cv.toDataURL('image/jpeg',0.75));
            URL.revokeObjectURL(img.src);
            _renderInvPhotoStrip();
        };
        img.src=URL.createObjectURL(f);
    });
};
window.invDelPhoto=function(i){ _invPhotos.splice(i,1); _renderInvPhotoStrip(); };
function _renderInvPhotoStrip(){
    const s=document.getElementById('invPhotoStrip');if(!s)return;
    s.innerHTML=_invPhotos.map((d,i)=>`
        <div style="position:relative;width:52px;height:52px;border-radius:8px;overflow:hidden;border:1.5px solid var(--g500)">
            ...
            <button onclick="invDelPhoto(${i})" style="position:absolute;top:1px;left:1px;width:17px;height:17px;border:none;border-radius:50%;background:rgba(239,68,68,.9);color:#fff;font-size:.6rem;font-weight:900;cursor:pointer;line-height:1;padding:0">✕</button>
        </div>`).join('');
}
function _invSavePhotos(iid){
    if(!_invPhotos.length)return 0;
    const n=_invPhotos.length;
    try{_db.ref('goldpro/_photos/'+iid).set({imgs:_invPhotos,ts:Date.now()});}catch(e){}
    _invPhotos=[];_renderInvPhotoStrip();
    return n;
}
